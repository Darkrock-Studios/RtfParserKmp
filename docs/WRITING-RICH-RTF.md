...
`rtf-writer` has two ways to produce RTF, for two different jobs:

| Path | Type | Use it when |
|---|---|---|
| `StyledDocument` → `RtfWriter` / `writeRtf` | minimal round-trip model | you're round-tripping reader output, or mapping a `Compose AnnotatedString` — bold/italic/underline runs and nothing else. See the [README](../README.md#write). |
| **`RtfDocument` → `RtfDocumentWriter` / `writeRtfDocument`** | **rich authoring model** | you're *generating* a document — fonts, colors, headings, page breaks, hyperlinks, bookmarks, paragraph layout. **This doc.** |

The rich model is a strongly-typed alternative to hand-concatenating control words: you describe the
document as immutable data and the writer serializes it. It lives entirely in `rtf-writer` and adds
nothing to `rtf-core`; the round-trip model above is untouched.

Like the minimal writer, output is **7-bit ASCII** — every code unit above 127 is emitted as a signed
`\uN` escape with a `?` fallback, so the result is charset-free and round-trips cleanly through the
reader.
