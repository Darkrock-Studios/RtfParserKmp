...
private external class TextDecoder(label: String) {
    fun decode(input: Int8Array): String
}
...
internal actual fun platformDecodeBytes(bytes: ByteArray, offset: Int, length: Int, charsetName: String): String {
    ...
    val slice = bytes.copyOfRange(offset, offset + length).unsafeCast<Int8Array>()
    return TextDecoder(label).decode(slice)
}
