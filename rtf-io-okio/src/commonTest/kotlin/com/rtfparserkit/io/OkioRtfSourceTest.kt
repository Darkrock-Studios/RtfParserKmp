...
class OkioRtfSourceTest {

    private fun buffer(rtf: String): Buffer = Buffer().apply { writeUtf8(rtf) }

    @Test
    fun extractsTextFromBufferSource() {
        val source = OkioRtfSource(buffer("""{\rtf1\ansi Hello, world!\par}"""))
    ...
    fun readSingleBytesWithPushback() {
        val src = OkioRtfSource(buffer("AB"))
        val a = src.read()
    ...
    fun bulkReadHonoursPushbackAndFillsFully() {
        val src = OkioRtfSource(buffer("HELLO"))
        val first = src.read()
    ...
    fun bulkReadReturnsShortCountOnlyAtEof() {
        val src = OkioRtfSource(buffer("XYZ"))
    ...
    fun unreadRejectsDoublePushback() {
        val src = OkioRtfSource(buffer("Z"))
        val z = src.read()
        src.unread(z)
        var threw = false
        try {
