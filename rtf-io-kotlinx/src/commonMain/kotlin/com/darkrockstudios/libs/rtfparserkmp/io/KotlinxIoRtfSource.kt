 ...
 */
class KotlinxIoRtfSource(private val source: Source) : RtfSource {
    ...
    override fun read(dest: ByteArray): Int {
        ...
        while (offset < dest.size) {
            val n = source.readAtMostTo(dest, offset, dest.size)
            if (n <= 0) break
            offset += n
        }
...
fun Source.asRtfSource(): RtfSource = KotlinxIoRtfSource(this)
